...
| Structured questions auto-submit an empty answer | Open Settings → Linux desktop and enable **Keep questions open**. The picker remains available, but pending requests stay open until you answer or interrupt them. |
...
Settings → Linux desktop. It disables only the Desktop auto-resolution timer;
the structured question UI and explicit answers remain enabled. The setting is
persisted in `~/.config/codex-desktop/settings.json` as
